<style>
    ...
    .f-input[list]::-webkit-calendar-picker-indicator {
...
═══════════════════════════════════════ */
    ...
    .img-box {
        ...
        width: 100%;
        padding-top: 100%;
        /* truco aspect-ratio 1:1 sin aspect-ratio */
        border-radius: 14px;
        ...
        overflow: hidden;
        /* TODO lo que salga adentro se recorta aquí */
    }
    ...
    .img-box input[type=file] {
        ...
        z-index: 5;
        font-size: 0;
        /* evita que algunos navegadores lo expandan */
    }
    ...
    .img-box:hover .img-placeholder span {
        color: var(--green);
    }

    .img-box.has-img .img-placeholder {
        display: none;
    }
    ...
    .img-del {
        ...
        justify-content: center;
        z-index: 6;
        /* encima del input para recibir clicks */
...
<div id="createPage" class="-mx-4 sm:-mx-6 lg:-mx-8 -mt-6">
    ...
    <main id="main">
        ...
        <div class="fade-up"
            style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:4px">
            <div>
                ...
                <p style="font-size:12px;color:var(--muted);margin-top:4px">Completa los campos para registrar un nuevo
                    producto en el catálogo Casatek</p>
            </div>
        ...
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" id="productForm">
            ...
            <div class="panel fade-up delay-1">
                <div class="panel-head">
                    <div
                        style="width:34px;height:34px;border-radius:10px;background:rgba(34,197,94,.15);display:flex;align-items:center;justify-content:center">
                ...
                <div class="panel-body">
                    <div class="form-grid" style="margin-bottom:20px">
                        <div class="form-group" style="grid-column:1/-1">
                            ...
                            @error('name')<p class="err-msg"><i
                            class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>@enderror
                        </div>
                        <div class="form-group">
                            <label class="form-label">SKU <span>*</span></label>
                            <input type="text" name="sku" value="{{ old('sku') }}" placeholder="Ej: SAM-TV55-4K"
                                class="f-input {{ $errors->has('sku') ? 'has-error' : '' }}"
                                style="font-family:monospace;letter-spacing:.05em">
                            <p class="hint"><i class="fa-solid fa-circle-info" style="font-size:10px"></i> Código único
                                del producto. No puede repetirse.</p>
                            @error('sku')<p class="err-msg"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}
                            </p>@enderror
                        </div>
                        <div class="form-group">
                            <label class="form-label">Días de garantía</label>
                            <input type="number" name="warranty_days" value="{{ old('warranty_days', 365) }}" min="0"
                                placeholder="365"
                                class="f-input {{ $errors->has('warranty_days') ? 'has-error' : '' }}">
                            @error('warranty_days')<p class="err-msg"><i
                            class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>@enderror
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        @error('description')<p class="err-msg"><i
                        class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>@enderror
                    </div>
                </div>
            </div>
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <div style="position:relative">
                                <span
                                    style="position:absolute;left:14px;top:50%;transform:translateY(-50%);color:var(--green);font-size:13px;font-weight:700">Bs.</span>
                                <input type="number" name="base_price" value="{{ old('base_price') }}" min="0"
                                    step="0.01" placeholder="0.00"
                                    class="f-input {{ $errors->has('base_price') ? 'has-error' : '' }}"
                                    style="padding-left:42px">
                            </div>
                            @error('base_price')<p class="err-msg"><i
                            class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>@enderror
                        </div>
                        <div class="form-group">
                            ...
                            @error('stock')<p class="err-msg"><i
                            class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>@enderror
                        </div>
                        <div class="form-group">
                            <label class="form-label">Marca</label>
                            <input type="text" name="brand_name" value="{{ old('brand_name') }}" list="brands-list"
                                placeholder="Selecciona o escribe una nueva..."
                                class="f-input {{ $errors->has('brand_name') ? 'has-error' : '' }}" autocomplete="off">
                            <datalist id="brands-list">
                                @foreach($brands as $brand)
                                    <option value="{{ $brand->name }}">
                                @endforeach
                            </datalist>
                            <p class="hint"><i class="fa-solid fa-circle-info" style="font-size:10px"></i> Elige una
                                existente o escribe una nueva para crearla.</p>
                            @error('brand_name')<p class="err-msg"><i
                            class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>@enderror
                        </div>
                        <div class="form-group">
                            ...
                            <p class="hint"><i class="fa-solid fa-circle-info" style="font-size:10px"></i> Elige una
                                existente o escribe una nueva para crearla.</p>
                            @error('category_name')<p class="err-msg"><i
                            class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>@enderror
                        </div>
                    </div>
                </div>
                        ...
                        @foreach(['image1', 'image2', 'image3', 'image4'] as $i => $field)
                            <div>
                                ...
                                <div class="img-box" id="box-{{ $field }}">
                                    ...
                                    <button type="button" class="img-del" onclick="imgRemove(event,'{{ $field }}')">
